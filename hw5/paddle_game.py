...
class PaddleGame(object):
    ...
    def set_max_tries(self, max_tries):
        self.MAX_TRIES_PER_GAME = max_tries
    ...
    def step(self, action):
        ...
        self.frames.append([self.ball_x, self.ball_y, self.paddle_x])
        
        if self.num_tries >= self.MAX_TRIES_PER_GAME: self.game_over = True
...
if __name__ == "__main__":   
    ...
    for e in range(NUM_EPOCHS):
        ...
        while not game_over:
            ...
            last4frames, reward, game_over = game.step(action)
            sys.stdout.write("\raction = %d, reward = %2d" % (action, reward))
