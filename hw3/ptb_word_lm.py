...
def get_prediction(session, model, inverseDictionary, data, eval_op=None, verbose=False):
  ...
  return np.exp(costs / iters)
    ...
    # # model is always the test data in this case.
    # # session has all the information from the past,
    # # and run on session?
    # for yconv in session.run(y_conv, feed_dict={x: model, keep_prob: 1.0}): 
...
def run_epoch(session, model, eval_op=None, verbose=False):
    ...
    print("the epoch size is: " + str(model.input.epoch_size))
    for step in range(model.input.epoch_size):
        ...
        # if step / (epoch_size // 10)
        if verbose and step % (model.input.epoch_size // 10) == 10:
            print("progress in this epoch: %.3f perplexity: %.3f speed: %.0f wps" %
                  (step * 1.0 / model.input.epoch_size, np.exp(costs / iters),
                   iters * model.input.batch_size * max(1, FLAGS.num_gpus) /
                   (time.time() - start_time)))

    # this returns the perplexity rate
    # how many costs over how many iterations
    return np.exp(costs / iters)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with sv.managed_session(config=config_proto) as session:
            ...
            test_perplexity = get_prediction(session, mtest, inverseDictionary, test_data)
