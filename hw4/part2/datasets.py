...
def loadStock(stock, window_size, predict_days, load_purpose=0):
    ...
    X = []
    weekly = []
    for i in range(0, len(values) - 5, 5):
        print('i is', i, 'values: ', values[i:i+5])
        weekly.append(sum(values[i:i + 5]) / 5)
    values = weekly
    for i in range(len(values) - window_size):
        X.append(values[i:i + window_size])
    Y = X[predict_days:]
    X = X[:-predict_days]
    print("before expansion: X.shape =", len(X), "Y.shape =", len(Y))
    X = np.expand_dims(X, axis=2)
    Y = np.expand_dims(Y, axis=2)

    print("after expansion: X.shape =", X.shape, "Y.shape =", Y.shape)
    return X, Y
