...
def play_a_game(isTest=2):
    ...
    for i in range(len(x0_)):
        if (i + to_predict) <= len(x0_):
            # warning: don't judge this beautiful code
            # i know I could do predicted[i:i+9] instead of 9 lines
            # but my brain is lazier than my fingers now
            x0 = x0_[i]
            x1 = predicted[i]
            x2 = predicted[i + 1]
            x3 = predicted[i + 2]
            ...
            x8 = predicted[i + 7]
            x9 = predicted[i + 8]
            indicator = buy_or_sell(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9)
